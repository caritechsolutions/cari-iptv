...
fix_permissions() {
    ...
    # Ensure packages directory exists for local TSDuck debs
...
setup_cron() {
    ...
    local CRON_FILE="/etc/cron.d/cari-iptv"
    local PHP_BIN
    PHP_BIN=$(which php 2>/dev/null || echo "/usr/bin/php")

    cat > "$CRON_FILE" << CRONEOF
# CARI-IPTV Scheduled Tasks
# Auto-generated by update.sh — do not edit manually

# EPG auto-fetch: runs every 5 minutes, only fetches sources due for refresh
*/5 * * * * $WEB_USER $PHP_BIN $INSTALL_DIR/scripts/epg-fetch.php >> $INSTALL_DIR/storage/logs/epg.log 2>&1
...
install_tsduck() {
    ...
    # STEP 2: If no local package, try downloading
