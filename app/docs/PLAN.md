...
- [ ] Repositories: auth, content, layout, epg, ads, recommendations, analytics
- [x] Unit tests: envelope, models (fixtures), url resolver, interceptor refresh flow
- [ ] Quality gates pass; commit

### P4 Auth UI
- [ ] Splash/session restore → login or home
- [ ] Login (identity/password, device_name, `device_type: mobile`), unverified-email notice with resend
- [ ] Register (all fields, validation), "check your email" screen
- [ ] Forgot password (email) and "link sent" screen; reset link opens the web page
- [ ] Logout (revokes refresh, clears storage); evicted-device handling on refresh 401
- [ ] Privacy and terms links on login and register
- [ ] Quality gates pass; commit

### P5 Server-driven home, navigation and pages
- [ ] Manifest poller + cache invalidation
- [ ] Bottom navigation from `/app/navigation/mobile` (max 5), page routing by `page_type`
- [ ] Layout renderer: `hero_slideshow`, `content_row` (server + client sources), `channel_grid`, `continue_watching`, `category_grid`, `banner`, `spotlight`, `text_divider`, recommendation sections, `live_now`, `epg_schedule`; unknown types skipped
- [ ] Built-in fallback home when `/app/layout/mobile` is 404
- [ ] Loading / empty / error / offline states
- [ ] Quality gates pass; commit

### P6 VOD
- [ ] Movies list (sort, category, pagination), Series list
- [ ] Movie detail (backdrop, metadata, cast, trailers link, rating, watchlist, play/resume)
- [ ] Series detail (seasons, episodes, per-episode progress via batch endpoint)
- [ ] Entitlement gating (`is_restricted` vs entitlements), adult gating with PIN
- [ ] Quality gates pass; commit

### P7 Player
- [ ] Playback screen (video_player), landscape fullscreen, wakelock, custom controls
- [ ] Resume prompt + progress POST every 10 s, completed handling
- [ ] Markers: skip intro, next-episode countdown at credits; `next_episode` chaining
- [ ] Subtitles (VTT from `subtitles[]`) toggle
- [ ] Error states with retry and the error code; QoE events
- [ ] Quality gates pass; commit

### P8 Live TV and EPG
- [ ] Channel list with categories and now/next
- [ ] Guide (grid by date) with programme detail
- [ ] Live playback (no resume), channel switching
- [ ] Quality gates pass; commit

### P9 Search, categories, watchlist, continue watching, profile, settings
- [ ] Search with type filter; Categories → filtered lists
- [ ] My List (watchlist) with detail hydration; Continue Watching
- [ ] Profile: account info, adult toggle, parental PIN, packages/entitlements (view only, no purchases)
- [ ] Settings: app version, clear cache, privacy/terms links, sign out, **Delete account** (confirmation + password re-entry)
- [ ] Quality gates pass; commit

### P10 Ads, recommendations, analytics
- [ ] Pre-roll / mid-roll (from `/ads/breaks`) with skip, impression + events; banner and text-scroller overlays per `/ads/overlay-settings`
- [ ] Recommendation sections; analytics event queue with batch flush (`platform` per flavour)
- [ ] Quality gates pass; commit
...
- [ ] `README.md` (setup, config, build, install), `STORE_READINESS.md`, `IOS_NEXT_STEPS.md`
